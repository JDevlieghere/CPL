...
;; Copied from Dieter's implementation
(define getsetfieldval
  (a-program
   ...
   (let-exp '(o2) 
            (list (new-object-exp 'c2 (list)))           
            (begin-exp
     (method-call-exp (var-exp 'o2) 'setx1 (list (const-exp 101))) ; set field x
     (list
      (fieldset-exp (var-exp 'o2) 'y (const-exp 102));'y1, set field y twice
      (fieldset-exp (var-exp 'o2) 'y (const-exp 999));'y2 
      (method-call-exp (var-exp 'o2) 'gety2 '()) ; gives num-val 999
      )))))
