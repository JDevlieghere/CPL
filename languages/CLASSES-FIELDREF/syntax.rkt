...
(define (exp->string exp)
  (cases expression exp
    ...
    (fieldset-exp (obj-exp field-name field-exp)
                  (string-append "set " (exp->string obj-exp) " field " (symbol->string field-name) " = " (exp->string field-exp)))
    (fieldref-exp (obj-exp field-name)
                  (string-append "ref " (exp->string obj-exp) " field " (symbol->string field-name)))
    ))
